...
gg_vankrev(percentile[percentile$Treatment=="PreFenton",], aes(x=OC,y=HC))+
  facet_wrap(~Forest)+
...
gg_vankrev(percentile[percentile$Treatment=="PostFenton",], aes(x=OC,y=HC))+
...
```{r sorbed_groups, fig.cap="", fig.width=9, fig.height=7}
sorbed_relabund %>% 
...
```{r goethite_vk2, fig.cap="native SOM", fig.width=5, fig.height=5}
# set order of levels
sorbed %>% 
  dplyr::mutate(fenton=factor(fenton, levels = c("PreFenton","PostFenton")))->sorbed
